%% MATLAB code for optomizing Condenser operating temp
...
for i = 1:50
    ...
    mdot_warm = 100; % kg/s
    cp = 4.186; % kJ/kg
    T1= 30; % Chosen from highest realistic temperature
    T2= 17; % C Flash chamber operating temperature
    T3 = T2;
    T5= i; % C Condenser operating temperature
    T8 = T5;
    Pchamber = 0.021; %[bar] Picked 0.021 based on optimal pressure
    T5Array(i) = T5; % collecting array for graph
...
figure
plot(T5Array,freshwatervalue)
xlabel('Temperature [C]')
ylabel('Fresh Water Dollar Value')

figure
plot(T5Array,powervalue)
xlabel('Temperature [C]')
ylabel(' Power Output Dollar Value')
